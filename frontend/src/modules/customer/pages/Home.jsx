...
const Home = () => {
  ...
  const fetchData = async ({ forceRefresh = false } = {}) => {
    ...
    try {
      ...
      if (catRes.data.success) {
        ...
        nextHomeData.quickCategories = dbCats.filter((cat) => cat.type === "category").map((cat) => ({ id: cat._id, name: cat.name, image: cat.image, priorityStartTime: cat.priorityStartTime, priorityEndTime: cat.priorityEndTime }));
      }
      if (prodRes.data.success) {
        ...
        nextHomeData.products = dbProds.map((p) => ({ ...p, id: p._id, image: p.mainImage || p.image || "https://images.unsplash.com/photo-1550989460-0adf9ea622e2?auto=format&fit=crop&q=80&w=400&h=400", price: p.customerSalePrice ?? p.customerPrice ?? p.salePrice ?? p.price, originalPrice: p.customerPrice ?? p.price, weight: p.weight || "1 unit", deliveryTime: p.deliveryEta?.label || "8-15 mins" }));
      }
  ...
  const hydrateSelectedSectionProducts = async (sections = []) => {
    ...
    try {
      ...
      // getProductById intentionally doesn't 404 on out-of-stock (a direct
      // product-page visit — e.g. from cart/wishlist/reorder — should still
      // show the page with an out-of-stock state). But this call is for
      // curating a home section's product grid, a discovery surface, so an
      // out-of-stock product picked up here must be dropped client-side.
      const fetchedMissing = missingResults.filter((r) => r.status === "fulfilled").flatMap((r) => { const p = r.value?.data?.result || r.value?.data?.results; return Array.isArray(p) ? p : (p ? [p] : []); }).filter((p) => Number(p?.stock || 0) > 0).map((p) => ({ ...p, id: p._id, image: p.mainImage || p.image || "https://images.unsplash.com/photo-1550989460-0adf9ea622e2?auto=format&fit=crop&q=80&w=400&h=400", price: p.customerSalePrice ?? p.customerPrice ?? p.salePrice ?? p.price, originalPrice: p.customerPrice ?? p.price, weight: p.weight || "1 unit", deliveryTime: p.deliveryEta?.label || "8-15 mins" }));
  ...
  const effectiveQuickCategories = useMemo(() => {
    ...
    let baseCategories = [];
    if (ids.length > 0) { 
      const resolved = ids.map((id) => categoryMap[id]).filter(Boolean).map((c) => ({ id: c._id, name: c.name, image: c.image, priorityStartTime: c.priorityStartTime, priorityEndTime: c.priorityEndTime }));
    ...
    <div className={`min-h-screen pt-[190px] md:pt-[250px] ${(needsLocationSetup || (!hasHydratedLocation || isLoading) || products.length === 0) ? "bg-white" : "bg-[#F5F7F8]"}`}>
      ...
      {needsLocationSetup ? (
        ...
        <>
              ...
              {heroConfig.banners?.items?.length ? (
                ...
                <div className="w-full h-[190px] bg-[#ecfeff] p-6 relative overflow-hidden flex items-center border-y border-primary/10 shadow-sm">
                  <div className="relative z-10 w-3/5 flex flex-col items-start gap-2">
                    ...
                    <button onClick={() => navigate('/stores')} className="bg-[#FF1E56] text-white px-6 py-2.5 rounded-2xl font-black text-xs tracking-wide active:scale-95 transition-transform">Order now</button>
                  </div>
          ...
          <div className="container mx-auto px-4 md:px-8 lg:px-[50px] py-3 md:py-4 flex justify-center sm:justify-start">
            <BecomeSellerButton />
          </div>
